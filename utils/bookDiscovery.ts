...
export function getBooks(): BookEntry[] {
  ...
  // Also import raw content for TSX to extract <h2> headings
  // (MDX headings are extracted at build time via the remark-export-headings plugin)
  ...
  discoveredFilesByBook.forEach((filesMap, bookSlug) => {
    ...
    sequence.forEach((item, index) => {
      ...
      // Helper to find the raw content regardless of query strings generated by Vite
      ...
      if (ext === 'mdx') {
        ...
        // Headings are injected as a named export by the remark-export-headings plugin
        const mdxHeadings = (m.headings as { title: string }[]) || [];
        mdxHeadings.forEach(h => headings.push(h));
      } else if (ext === 'tsx') {
        component = m.default;
      ...
      // Extract headings from rawContent (TSX and HTML only;
      // MDX headings were already added above from m.headings)
      if (rawContent && rawContent.length > 0) {
        // HTML / TSX: match <h2>...</h2> tags
        const htmlRegex = /<h2[^>]*>([\/\s\S]*?)<\/h2>/gi;
        let match;
        while ((match = htmlRegex.exec(rawContent)) !== null) {
          const hTitle = match[1].replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim();
          if (hTitle) headings.push({ title: hTitle });
        }
      }
